...
(when (version< so-long-version so-long--latest-version)
  ...
  (when (version< so-long-version "1.1")
    ...
    (let ((saved (eval (car (get 'so-long-predicate 'saved-value))))
          (standard (eval (car (get 'so-long-predicate 'standard-value)))))
      (when (and (eq standard 'so-long-statistics-excessive-p)
                 (or (not saved)
                     (eq saved 'so-long-detected-long-line-p)))
        (setq so-long-predicate standard))))
